...
| Module | File | Purpose |
| :--- | :--- | :--- |
| B1 | `mr_engine/llm_factory.py` | Multi-model routing factory (Gemini/OpenAI/Anthropic/vLLM) |
| B2 | `mr_engine/transforms/__init__.py` | Decorator-based registry (13 transforms) |
| B2 | `mr_engine/transforms/vcf.py` | 9 VCF atomic transforms |
| B2 | `mr_engine/transforms/sam.py` | 4 SAM atomic transforms |
| B3 | `mr_engine/behavior.py` | 6 behavior target categories + prompt fragments |
| B5 | `mr_engine/dsl/models.py` | Pydantic models + deterministic MD5 hashing |
| B5 | `mr_engine/dsl/compiler.py` | Validation + ChromaDB metadata hydration |
| B4 | `mr_engine/agent/tools.py` | SpecIndex -> LangChain tool bridge |
| B4 | `mr_engine/agent/prompts.py` | System prompt builder (`mr_name`, not `mr_id`) |
| B4 | `mr_engine/agent/engine.py` | ReAct agent + 3-retry validation loop |
| B6 | `mr_engine/registry.py` | Enforced/Quarantine triage + dedup by `mr_id` |
| CLI | `mr_engine/__main__.py` | `python -m mr_engine` entry point |
| Config | `.env.example` | API key template |
| Tests | `tests/test_transforms.py`, `test_dsl.py`, `test_registry.py` | 70 tests |
