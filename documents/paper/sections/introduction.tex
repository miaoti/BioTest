...
end-of-run line coverage, and a silence-on-fix
benchmark~\citep{boehme2021estimating} of $32$ verified parser
bugs (a fix is judged to silence a tool's signal iff the same
input that triggers it on the pre-fix version no longer triggers
it on the post-fix version). \biotest{} is competitive with the
...
Detection is end-to-end audited under the silence-on-fix
predicate of B\"ohme et al.~\cite{boehme2021estimating} on
inputs the tool's adapter produced during the per-cell budget,
not on the manifest's canonical proof-of-vulnerability files
alone.
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
  ...
  \item A three-axis evaluation (mutation adequacy, line
        coverage, $32$-bug silence-on-fix benchmark) under matched
