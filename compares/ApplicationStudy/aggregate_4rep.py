...
def _coverage_data(config_name, sub, results_root, ids, layout):
    ...
    or per-rep coverage (E2/E3).

    Missing-rep handling:
      * `status=="missing"` → 0.0 (ablation produced no tests, see _rep_value).
      * No measurement.json file at all → ALSO treated as 0.0 for the
        same reason: under E1S/E3 the rerun pipeline can produce 0 MRs +
        0 measurement (Phase B fails, harness exits with no JaCoCo flush
        AND no run_4rep post-rep write). For E0 we never observe this.
        Counting completely-missing big_runs as 0 keeps n=4 honest about
        the ablation's failure rate rather than silently dropping them.
    """
    out = {}
    for sut, fmt in CELLS:
        ...
        for run_id in ids:
            if layout == "big":
                # Per-big-run mean across 3 reps. If 0 reps recoverable,
                # treat the big_run as 0 (ablation outcome).
                rep_vals = []
                for rep in range(3):
                    ...
                    try:
                        ...
                        continue
                    v = _rep_value(d)
                    rep_vals.append(v if v is not None else 0.0)
                ...
                try:
                    ...
                    continue
                v = _rep_value(d)
                per_run.append(v if v is not None else 0.0)
        out[cell] = per_run
    return out
...
def main():
    ...
    lines = ["# 4-Rep Application Study — E0 vs E1S vs E2 vs E3 SUMMARY", ""]
    ...
    lines.append("E1S = no Phase A (blind LLM baseline — no spec content in prompt)")
    lines.append("E2 = no Phase D (single-shot B+C+E, RAG enabled)")
    lines.append("E3 = no Phase A AND no Phase D")
    lines.append("")
    ...
    lines.append("| Cell | E0 | E1S | E2 | E3 |")
    lines.append("| :--- | :--- | :--- | :--- | :--- |")
    for sut, fmt in CELLS:
        cell = f"{sut}_{fmt.lower()}"
        row = [f"| **{cell}** |"]
    ...
    lines.append("| Run idx | E0 | E1S | E2 | E3 |")
    lines.append("| :--- | :--- | :--- | :--- | :--- |")
