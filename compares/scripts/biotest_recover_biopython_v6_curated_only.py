...
def main():
    ...
    log_path = out_root / "recovery_biopython_v6.log"
    fh = log_path.open("a", encoding="utf-8", buffering=1)
    log("=== biopython_sam v6 curated-only recovery start ===", fh)
    ...
    try:
        for rep in REPS:
            ...
            cfg = rep_dir / f"biotest_config.recover_v6_rep{rep}.yaml"
            log(f"  writing config -> {cfg.name}", fh)
            write_config(rep, cfg)

            biotest_log = rep_dir / "biotest_recover_v6.log"
            ...
            rec = {
                ...
                "rep_kind": "recovery_v6_curated_only",
