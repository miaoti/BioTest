...
# Recover biopython_sam reps 0..3 — all 4 hit Windows TerminateProcess kill at
# the 5400s wall cap because the new SAM transforms made Phase D slower;
# coverage.py atexit never fired. Each cell preserved its post-rep-N seed
# corpus under work/seeds/, so we re-run individually with max_iter=1 (graceful
# exit ~30-50 min) reusing whatever cascaded corpus the failed orchestrator
# left. seqan3_sam succeeded via the retry orchestrator with the longer
# subprocess buffer; htsjdk_sam succeeded all 4 reps under the original cascade
# (JaCoCo is incremental, no atexit dependency).
TARGETS = [
    {"cell": "biopython_sam", "sut": "biopython", "fmt": "SAM",
     "reps_to_redo": [0, 1, 2, 3], "kind": "coveragepy"},
]
...
def write_recovery_config(target: dict, work_dir: Path, rep: int,
    ...
    # Inner timeout 70 min < outer 9000s = 150 min; gives coverage.py atexit
    # ample margin to flush its SQLite even under the heavier MR menu (the
    # original 80/4500 ordering was inverted — outer fired first and killed
    # biotest before atexit).
...
def recover_cell(target: dict, fh) -> list[dict]:
    ...
    for rep in target["reps_to_redo"]:
        ...
        rc, elapsed = run_biotest(cfg_path, biotest_log, budget_s=9000)
