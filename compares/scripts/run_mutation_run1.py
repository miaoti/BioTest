...
REPS = [0, 1, 2, 3]
RAWFUZZ_SEEDS = [0, 42, 100, 200]  # per-rep Rank 13 seed; varies the
                                     # only stochastic Phase-E layer
...
def write_report(agg: dict) -> None:
    ...
    lines = [
        "# BioTest Run-10 — 4-rep mean ± std on auto-pipeline corpus",
        "",
        f"4 reps with different Rank 13 lenient-byte-fuzz seeds ({RAWFUZZ_SEEDS}).",
    ...
    out_path = REPO_ROOT / "compares/results/mutation/biotest/RUN10_AUTO_PIPELINE_4REP.md"
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    log(f"wrote {out_path}")
    raw_path = REPO_ROOT / "compares/results/mutation/biotest/run10_raw.json"
