...
CELL_DIR = (
    ...
    / "biotest_4rep_cascade_sam_round4_autoinvoke_20260501" / "biopython_sam"
)
...
TARGET_FMT = "SAM"
REPS = [0, 1, 2]  # 3 independent reps for honest std band vs SOTA
...
def main():
    ...
    log_path = out_root / "recovery_biopython_v8.log"
    fh = log_path.open("a", encoding="utf-8", buffering=1)
    log("=== biopython_sam v8 curated-only recovery start ===", fh)
    ...
    try:
        for rep in REPS:
            ...
            cfg = rep_dir / f"biotest_config.recover_v8_rep{rep}.yaml"
            log(f"  writing config -> {cfg.name}", fh)
            write_config(rep, cfg)

            biotest_log = rep_dir / "biotest_recover_v8.log"
            ...
            rec = {
                ...
                "rep_kind": "recovery_v8_max_iter_2",
