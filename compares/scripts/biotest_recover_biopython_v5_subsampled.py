#!/usr/bin/env python3
"""Subsampled biopython_sam recovery — v5.

v4 stalled because 750 cascaded SAM seeds × 8 MRs ≈ 4 hr/rep, exceeding the
150-min outer budget; biotest's timeout_minutes is checked at iteration
boundaries (not seed boundaries) so with max_iter=1 the inner timeout never
fires within iter 1. Outer wall cap kills biotest before coverage.py atexit
flushes -> status=missing.

v5 subsamples the seed corpus per rep (similar to biotest_recover_vcfpy_only.py):
  - keep ALL non-kept_* seeds (curated/external/synthetic — high-coverage shapes)
  - random sample N kept_* per rep, RNG seed = 42 + rep (gives meaningful std)
After recovery, the full corpus is restored from the backup.
"""
...
TARGET_KEPT = 150  # subsample this many kept_* per rep
...
def main():
    ...
    log_path = out_root / "recovery_biopython_v5.log"
    fh = log_path.open("a", encoding="utf-8", buffering=1)
    log("=== biopython_sam v5 subsampled recovery start ===", fh)
    ...
    try:
        for rep in REPS:
            ...
            cfg = rep_dir / f"biotest_config.recover_v5_rep{rep}.yaml"
            log(f"  writing config -> {cfg.name}", fh)
            write_config(rep, cfg)

            biotest_log = rep_dir / "biotest_recover_v5.log"
            ...
            rec = {
                ...
                "rep_kind": "recovery_v5_subsampled",
